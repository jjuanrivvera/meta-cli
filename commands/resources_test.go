...
func TestResourceCommandsDryRun(t *testing.T) {
	...
	cases := [][]string{
		...
		{"instagram", "insights", "account"}, {"instagram", "insights", "media", "media-1"}, {"instagram", "limits", "publishing"},
		...
		{"pages", "videos", "thumbnail", "video-1", "--url", "https://cdn.example/thumb.jpg"},
		{"pages", "videos", "publish", "--file", mediaPath, "--thumbnail-url", "https://cdn.example/thumb.jpg"},
		...
		{"pages", "insights", "page"}, {"pages", "insights", "post", "post-1"},
...
func TestEveryAPICommandIsAnnotated(t *testing.T) {
	...
	local := map[string]bool{"auth": true, "config": true, "init": true, "doctor": true, "completion": true, "alias": true, "version": true, "mcp": true, "agent": true, "update": true}
