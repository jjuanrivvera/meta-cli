...
Render host policy that blocks irreversible operations

```
metactl agent guard [flags]
```

### Examples

```
  metactl agent guard --host codex
...
```
      --all-writes    block ordinary writes as well as irreversible operations
      --dir string    project directory for --write (default ".")
  -h, --help          help for guard
      --host string   target host: claude-code, codex, or opencode
      --write         write files without replacing existing files
```
