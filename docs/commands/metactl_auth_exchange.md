## metactl auth exchange

Exchange the active user token for a long-lived token

```
metactl auth exchange [flags]
```

### Examples

```
  metactl auth exchange --save
```
...
```
  -h, --help   help for exchange
      --save   replace the stored token after a successful exchange
```
