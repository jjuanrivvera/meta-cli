...
accepted only through a hidden prompt or `METACTL_APP_SECRET`, and secret flags are not exported
as MCP tool inputs. MCP file inputs are confined to `METACTL_MCP_ROOT` (or the server working
